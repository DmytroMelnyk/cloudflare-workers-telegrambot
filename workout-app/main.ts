...
class DemoAppType {
    ...
    sendMessage(msg_id, with_webview) {
        ...
        const btn = document.querySelector<HTMLButtonElement>('#btn_status');
    ...
    changeMenuButton(close) {
        document.querySelectorAll('button').forEach((btn) => btn.disabled = true);
        const btnStatus = document.querySelector<HTMLButtonElement>('#btn_status');
    ...
    sendText(spam) {
        const textField = document.querySelector<HTMLInputElement>('#text_field');
    ...
    showPopup() {
        ...
        Telegram.WebApp.showPopup({
            ...
            if (buttonId === 'delete') {
                self.showAlert("'Delete all' selected");
...
class DemoAppMenuType {
    constructor(private demoApp: DemoAppType) {
    }
...
class DemoAppInitDataType {
    ...
    init() {
        ...
        document.getElementById('webview_data').innerHTML = JSON.stringify(this.demoApp.initDataUnsafe, null, 2);
        document.getElementById('theme_data').innerHTML = JSON.stringify(Telegram.WebApp.themeParams, null, 2);
        this.demoApp.checkInitData();
    }
}

class DemoAppViewportType {
    constructor(private demoApp: DemoAppType) {
    }

    init() {
        this.demoApp.init();
        Telegram.WebApp.onEvent('viewportChanged', this.setData);
        this.setData();
    }
...
const DemoApp = new DemoAppType();
const DemoAppMenu = new DemoAppMenuType(DemoApp);
const DemoAppInitData = new DemoAppInitDataType(DemoApp);
const DemoAppViewport = new DemoAppViewportType(DemoApp);
